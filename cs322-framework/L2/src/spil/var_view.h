...
namespace L2
{
	namespace Spil
	{
		...
		struct VarViewer
		{
			...
			std::string_view operator()( const nameNode &name_n )
			{
				return name_n.val;
			}

			std::string_view operator()( const varNode &var_n )
			{
			...
			std::string_view operator()( const KWNode &kw_n )
			{
				return KWNode::kw;
			}
			...
			std::string_view operator()( const VariantNode &variant_n )
			{
			...
			std::string_view operator()( const Node &n  )
			{
				return L2::EMPTYTOK;
			}
